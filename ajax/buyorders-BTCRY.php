...
$sql = "SELECT * FROM buy_orderbook WHERE want='".$my_coins->coins_names_prefix[1]."' and trade_id='".$my_coins->coins_names_prefix[0]."_".$my_coins->coins_names_prefix[1]."_".$my_coins->coins_names_prefix[2]."' and processed='1'";
$result = mysql_query($sql);
$count = mysql_num_rows($result);
if($count!=0) {
   $Query = mysql_query("SELECT action, amount, rate FROM buy_orderbook WHERE want='".$my_coins->coins_names_prefix[1]."' and trade_id='".$my_coins->coins_names_prefix[0]."_".$my_coins->coins_names_prefix[1]."_".$my_coins->coins_names_prefix[2]."' and processed='1' ORDER BY rate DESC");
   while($Row = mysql_fetch_assoc($Query)) {
      ...
      $buy_subtotal = "0";
      $Query = mysql_query("SELECT amount,rate FROM buy_orderbook WHERE want='".$my_coins->coins_names_prefix[1]."' and trade_id='".$my_coins->coins_names_prefix[0]."_".$my_coins->coins_names_prefix[1]."_".$my_coins->coins_names_prefix[2]."' and processed='1' ORDER BY rate ASC");
      while($Row = mysql_fetch_assoc($Query)) {
