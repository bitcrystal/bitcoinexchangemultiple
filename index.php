...
if(isset($_GET['recover']) && isset($_GET['username']) && isset($_GET['id']) )
{
	if($_GET['recover']==1)
	{
		...
		$id = $_GET['id'];
		$sql = "SELECT * FROM users WHERE username='$username' AND trade_id = '".$my_coins->coins_names_prefix[0]."_".$my_coins->coins_names_prefix[1]."_".$my_coins->coins_names_prefix[2]."'";
		...
		if($result)
		{
			...
			if($count == 1)
			{
				...
				if($my_id==$id)
				{
					$sql = "UPDATE users SET password = '".md5($Row['password'])."' WHERE username = '".$Row['username']."' AND trade_id = '".$my_coins->coins_names_prefix[0]."_".$my_coins->coins_names_prefix[1]."_".$my_coins->coins_names_prefix[2]."';";
					if(mysql_query($sql))
					{
...
if($form_action=="forgot") {
	if($email) {
		$sql = "SELECT * FROM users WHERE email='$email' AND trade_id = '".$my_coins->coins_names_prefix[0]."_".$my_coins->coins_names_prefix[1]."_".$my_coins->coins_names_prefix[2]."'";
        $result = mysql_query($sql);
		if($result)
		{
			$count = mysql_num_rows($result);
   ...
   if($login_attempts<=5) {
      if($myusername) {
         if($mypassword) {
            ...
            $sql = "SELECT * FROM users WHERE username='$myusername' AND trade_id = '".$my_coins->coins_names_prefix[0]."_".$my_coins->coins_names_prefix[1]."_".$my_coins->coins_names_prefix[2]."'";
            $result = mysql_query($sql);
            $count = mysql_num_rows($result);
            if($count==1) {
               $Pass_Query = mysql_query("SELECT * FROM users WHERE username='$myusername' AND trade_id = '".$my_coins->coins_names_prefix[0]."_".$my_coins->coins_names_prefix[1]."_".$my_coins->coins_names_prefix[2]."'");
            ...
            if($return_error == "") {
               if($db_found) {
                  ...
                  if($num_rows==1) {
                     ...
                     if(!mysql_query("INSERT INTO users (id,date,ip,username,password,email,trade_id) VALUES ('','$date','$ip','$myusername','$mypassword', '$email', '".$my_coins->coins_names_prefix[0]."_".$my_coins->coins_names_prefix[1]."_".$my_coins->coins_names_prefix[2]."')")){
                        $return_error = "System error.";
						$_SESSION['user_time'] = 0;
                     } else {
                        $return_error = "Logged in.";
